...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def maxValue(self, gameState,depth,agentIndex,alpha, beta):
        ...
        for action in actions:
            ...
            v = max(v,self.minValue(successor,depth,1,alpha,beta))
            if v > beta:
                return v
            alpha = max(alpha,v)
        return v
    ...
    def minValue(self, gameState,depth,agentIndex,alpha,beta):
        ...
        for action in actions:
            ...
            if agentIndex == (gameState.getNumAgents()-1):
                v = min(v,self.maxValue(successor,depth+1,0,alpha,beta))
                if v < alpha:
                    return v
                beta = min(beta,v)
            else:
                v = min(v,self.minValue(successor,depth,agentIndex+1,alpha,beta))
                if v < alpha:
                    return v
                beta = min(beta,v)
        return v
